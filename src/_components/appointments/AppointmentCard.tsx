...
const AppointmentCard: React.FC<AppointmentCardProps> = ({
  ...
  console.log(ianaTimezone[0] ?? clientTimezone);
            ...
            <div className="flex flex-col space-y-1">
              ...
              <h3 className="hidden lg:flex">
                <span className="first-letter:capitalize">
                  {`${format(zonedStartTime, "EEEE d 'de' LLLL 'de' yyyy', de' H:mm", { locale: es })} a ${zonedEndTime}hs`}
                </span>
              </h3>

              <h3 className="first-letter:capitalize lg:hidden">
                {`${format(zonedStartTime, "d 'de' LLL 'de' yyyy', ' H:mm", { locale: es })} a ${zonedEndTime}`}
              </h3>
            </div>
          </div>
