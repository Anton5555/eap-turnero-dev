"use client";

import * as React from "react";

import {
  type CaptionProps,
  type DayContentProps,
  DayPicker,
  useNavigation,
} from "react-day-picker";
import { es } from "date-fns/locale";

import { cn, parseDateWithPreservedTimezone } from "~/lib/utils";
import { buttonVariants } from "~/_components/common/Button";
import { format } from "date-fns";
import ChevronLeft from "../icons/ChevronLeft";
import ChevronRight from "../icons/ChevronRight";
import { H6 } from "./Typography";

function CustomCaption(props: CaptionProps) {
  const { goToMonth, nextMonth, previousMonth } = useNavigation();

  return (
    <div className="flex w-full justify-between">
      <button
        disabled={!previousMonth}
        onClick={() => previousMonth && goToMonth(previousMonth)}
      >
        <ChevronLeft />
      </button>

      <H6 className="capitalize">
        {format(props.displayMonth, "LLLL", { locale: es })}
      </H6>

      <button
        disabled={!nextMonth}
        onClick={() => nextMonth && goToMonth(nextMonth)}
      >
        <ChevronRight />
      </button>
    </div>
  );
}

function CustomDayContent({ activeModifiers, date }: DayContentProps) {
  const { today, hasEvent, outside } = activeModifiers;

  return (
    <div
      className={cn(
        today && "bg-green text-white",
        "flex h-full flex-col items-center justify-center rounded-md",
      )}
    >
      {hasEvent && (
        <div
          className={cn(
            "h-2 w-2 rounded-full bg-green",
            today && "bg-white",
            outside && "bg-gray-400",
          )}
        />
      )}

      <p className={cn(outside && "text-gray-400")}>{date.getDate()}</p>
    </div>
  );
}

export type CalendarProps = React.ComponentProps<typeof DayPicker> & {
  daysWithEvents?: string[];
};

const SUNDAY = 0;

const NextAppointmentsCalendar: React.FC<CalendarProps> = ({
  className,
  classNames,
  showOutsideDays = true,
  daysWithEvents = [],
  ...props
}) => (
  <DayPicker
    showOutsideDays={showOutsideDays}
    modifiers={{
      hasEvent: daysWithEvents.map((date) =>
        parseDateWithPreservedTimezone(date),
      ),
    }}
    className={cn("p-3", className)}
    weekStartsOn={SUNDAY}
    classNames={{
      months: "flex flex-col sm:flex-row space-y-4 sm:space-x-4 sm:space-y-0",
      month: "space-y-4",
      caption: "flex justify-center pt-1 relative items-center",
      nav: "space-x-1 flex items-center",
      nav_button: cn(
        buttonVariants({ variant: "outline" }),
        "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100",
      ),
      nav_button_previous: "absolute left-1",
      nav_button_next: "absolute right-1",
      table: "w-full border-collapse space-y-1",
      head_row: "flex",
      head_cell:
        "text-muted-foreground rounded-md w-9 font-normal text-[0.8rem] capitalize",
      row: "flex w-full mt-2",
      cell: "h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
      day_range_end: "day-range-end",
      day: "h-full",
      day_selected:
        "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground",
      day_today: "bg-accent text-accent-foreground",
      day_outside:
        "day-outside text-muted-foreground opacity-50 aria-selected:bg-accent/50 aria-selected:text-muted-foreground aria-selected:opacity-30",
      day_disabled: "text-muted-foreground opacity-50",
      day_range_middle:
        "aria-selected:bg-accent aria-selected:text-accent-foreground",
      day_hidden: "invisible",
      ...classNames,
    }}
    locale={es}
    components={{
      IconLeft: () => <ChevronLeft />,
      IconRight: () => <ChevronRight />,
      Caption: CustomCaption,
      DayContent: CustomDayContent,
    }}
    {...props}
  />
);

NextAppointmentsCalendar.displayName = "NextAppointmentsCalendar";

export default NextAppointmentsCalendar;
